...
public class portal_MIDIncrmnt {
	...
	public void Device_Assign() throws Exception {
		...
		//TestResultUtils.logger = TestResultUtils.extent.startTest("MID Increment Scenario");
		...
		try {
			...
			TID1= TidValue+1;
			if(library.sendText(objects.Tid_field, TID1) == true) {
		...
		try {
			...
			TestResultUtils.logger.log(LogStatus.PASS, "Successfully uploaded MID's and TID- 1" +TestResultUtils.logger.addScreenCapture(library.takescreenshot("Successfully uploaded MID's and TID's")));
			/*}else
